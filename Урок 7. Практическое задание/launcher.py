...
while True:
    ...
    if ACTION == 'q':
        ...
        for i in range(2):
            PROCESS.append(subprocess.Popen(f'python client.py -m send -n sends_client_{i+1}',
                                            creationflags=subprocess.CREATE_NEW_CONSOLE))
        for i in range(3):
            PROCESS.append(subprocess.Popen(f'python client.py -m listen -n listens_client_{i+1}',
                                            creationflags=subprocess.CREATE_NEW_CONSOLE))
    elif ACTION == 'x':
        while PROCESS:
