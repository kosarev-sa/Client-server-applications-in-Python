...
@LogDecCls()
def client_message_validator(msg, msg_lst, client):
    ...
    # Если это сообщение не о присутствии, то добавляет его в очередь сообщений. Ответ не требуется
        ...
        msg_lst.append((msg[ACCOUNT_NAME], msg[MESSAGE_TEXT]))
        return
    ...
    while True:
        ...
        if recv_lst:
            for r_client in recv_lst:
                try:
                    client_message_validator(get_message(r_client), messages, r_client)
                except:
                    log.info(f'Клиент {r_client.getpeername()} отключился от сервера')
                    clients.remove(r_client)

        # Если есть сообщения для отправки и ожидающие клиенты, отправляет им сообщение
        if messages and send_lst:
            ...
            del messages[0]
            for w_client in send_lst:
                try:
                    send_message(w_client, message)
                except:
                    log.info(f'Клиент {w_client.getpeername()} отключился от сервера')
